    ...
    def __init__(self, img_dir, batch_size, image_size, box_size):
        ...
        def _parser(record):
            keys_to_features = {
                ...
                'heatmaps': tf.VarLenFeature(dtype=tf.float32)}
            ...
            heatmap = tf.sparse_tensor_to_dense(parsed["heatmaps"])
            heatmap = tf.reshape(heatmap, [ self.box_size, self.box_size, self.total_joints+1])
            #joints = tf.sparse_tensor_to_dense(parsed['joint'])
            #joints = tf.reshape(joints, [self.total_joints, 2])
